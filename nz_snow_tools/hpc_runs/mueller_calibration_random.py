...
n_runs = 2000
ensemble_id = 'test_randomE'
deltaT_list = [1,2,3,4,5] # air temperature offsets to test
...
for deltaT in deltaT_list:
    ...
    inp_ta = aws_df.tc[start_t:end_t][:, np.newaxis] * np.ones(grid_size) + 273.16  + deltaT # 2 metre air temp in C
    ...
    if model == 'clark':
        ...
        random_params = {
            ...
            'mf_alb_decay':[1, 10],
            'mf_ros':[0, 5],
            # 'mf_doy_max_ddf':[], # keep at dec for now as no valid reason to vary
            'mf_doy_min_ddf':[170,250] # allow to vary to account for cold content of snowpack
        }
    ...
    pkl.dump(stor_dict, open(outfolder + '/collated_output_{}_deltaT{}.pkl'.format(ensemble_id,deltaT), 'wb'), -1)
