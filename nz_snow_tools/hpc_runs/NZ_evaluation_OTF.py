...
def NZ_evaluation(run_id, met_inp, which_model, hydro_years_to_take, catchment, output_dem, origin, model_swe_sc_threshold, mask_file, dsc_snow_output_folder,
                  output_folder):
    ...
    for hydro_year_to_take in hydro_years_to_take:
        ...
        # cut to same shape as trimmed_MODIS mask (5785, 4009), compared to trimmed nz mask (5901,4009), the N edge is further south in modis, so remove last 116 rows
        st_swe = st_swe[:, :-116, :]
...
if __name__ == '__main__':
    ...
    NZ_evaluation(run_id, met_inp, which_model, hydro_years_to_take, catchment, output_dem, origin, model_swe_sc_threshold, mask_file, dsc_snow_output_folder,
                  output_folder)
