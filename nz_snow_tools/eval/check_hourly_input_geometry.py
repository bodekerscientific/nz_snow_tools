...
climate_file = nc.Dataset(r"T:\DSC-Snow\input_data_hourly\met_inp_Nevis_nztm250m_2000_norton.nc")
...
for y_point in range(120,130):
    ...
    print('slope = {} aspect = {}'.format(slope, aspect))
    plt.plot(nc_dt,sw_in,'o-',label = 'slope = {:2.0f} aspect = {:3.0f}'.format(slope, aspect))
...
plt.figure()
plt.imshow(np.rad2deg(topo_file.variables['aspect']),origin=0)
plt.contour(topo_file.variables['DEM'][:],range(0,3000,100),colors='k',linewidths=.5)
