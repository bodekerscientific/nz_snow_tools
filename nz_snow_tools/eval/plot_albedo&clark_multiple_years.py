...
for i in range (0,7) :
    ...
    # observed data
    # clark
    inp_clark_obs = np.load(clark_file_obs.format(Stname[0],Stname[0],Stname[0],year), allow_pickle=True)
    ...
    # albedo
    inp_alb_obs = np.load(albedo_file_obs.format(Stname[0],Stname[0],Stname[0],year), allow_pickle=True)
    ...
    # albedo
    inp_alb_precip_ta = np.load(albedo_file_precip_ta.format(Stname[0],Stname[0],Stname[0],year), allow_pickle=True)
    ...
    # albedo
    inp_alb_precip_rad = np.load(albedo_file_precip_rad.format(Stname[0],Stname[0],Stname[0],year), allow_pickle=True)
    ...
    # albedo
    inp_alb_precip_rad_ta = np.load(albedo_file_precip_rad_ta.format(Stname[0],Stname[0],Stname[0],year), allow_pickle=True)
    ...
    # albedo
    inp_alb_precip = np.load(albedo_file_precip.format(Stname[0],Stname[0],Stname[0],year), allow_pickle=True)
...
f2, axs2 = plt.subplots(3, 3, figsize=(10, 10))  # sets number of rows and columns of subplot as well as figure size in inches
axs = axs2.ravel()
for j in range(9):  # run through each subplot (here there are 9 because of 3 rows and columns)
    ...
    plt.plot(inp_dt1_obs, inp_swe1_obs,'*', label = 'Obs precip')
    ...
    # plt.legend()
    # plt.savefig("C:/Users/Bonnamourar/OneDrive - NIWA/SIN calibration timeseries/Mueller/Mueller_Plots/{}_{}".format(Stname[0], year))
    ...
    plt.show()
    plt.close()
f2.savefig("C:/Users/Bonnamourar/OneDrive - NIWA/SIN calibration timeseries/Mueller/Mueller_Plots/{}".format(Stname[0]))  # save the figure once it’s done.