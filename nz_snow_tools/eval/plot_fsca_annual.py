...
for run_id in run_ids:
    ...
    for subcatchment in output_subcatchments:
        ...
        modis_fscd = resample_to_fsca(modis_sub/365.,rl=4)
        mod1_fscd = resample_to_fsca(mod1_sub/365.,rl=4)

        plt.subplot(1, 3, 1)
        cmap = plt.cm.viridis
        cmap.set_bad('0.5')
        plt.imshow(modis_fscd, origin=0, interpolation='none', vmin=0, vmax=1)
        plt.colorbar()
        plt.title('mean fsca (for snow = modis fsca > {})'.format(modis_sc_threshold))

        plt.subplot(1, 3, 2)
        plt.imshow(mod1_fscd, origin=0, interpolation='none', vmin=0, vmax=1)
        plt.colorbar()
        plt.title('mean fsca (for snow = dsc_snow SWE thres= {}'.format(model_swe_sc_threshold))

        plt.subplot(1, 3, 3)
        plt2 = mod1_fscd - modis_fscd
        # plt2[(plt2 > 100)] = 0  # some bad model points
        cmap = plt.cm.RdBu
        cmap.set_bad('0.5')
        plt.imshow(plt2, origin=0, interpolation='none', vmin=-0.5, vmax=0.5,cmap=cmap)  # , vmin=0, vmax=365, cmap='viridis')
        plt.colorbar()
        plt.title('difference  in mean fsca (mod-obs)')

        plt.tight_layout()
        plt.savefig(
            plot_folder + '/FSCA{}to{}_sub_{}_{}_thres{}_swe{}_{}_{}.png'.format(years_to_take[0], years_to_take[-1], subcatchment, output_dem,
