...
if __name__ == '__main__':
    ...
    for modis_sc_threshold in modis_sc_thresholds:
        ann = pickle.load(
            open(model_output_folder + '/summary_{}_{}_thres{}_swe{}.pkl'.format(catchment, output_dem, modis_sc_threshold, model_swe_sc_threshold), 'rb'))
        ann_ts_av_sca_m_thres = ann[14]
        ann_ts_av_sca_m_thres[5][:] = np.nan  # years 2006 and 2011 have bad modis data
        ann_ts_av_sca_m_thres[10][:] = np.nan
        ann_ts_av_sca_m_thres_array = np.zeros((len(ann_ts_av_swe), 365))
    ...
    plt.ylabel('SCA')
    plt.legend()
    plt.savefig(plot_folder + '/average snow covered area HY {}-{} swe{}.png'.format(years_to_take[0], years_to_take[-1], model_swe_sc_threshold))
